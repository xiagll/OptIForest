...
class LSHiForest(LSHForest):
	...
	def __init__(self, lsh_family='L2SH', num_trees=100, num=403, branch=0, granularity=1):
		if lsh_family == 'ALSH':
			LSHForest.__init__(self, num_trees, VSSampling(num_trees), AngleLSH(), num, branch, distance.cosine, granularity)
		elif lsh_family == 'L1SH':
			LSHForest.__init__(self, num_trees, VSSampling(num_trees), E2LSH(norm=1), num, branch, distance.cityblock, granularity)
		elif lsh_family == 'KLSH':
			LSHForest.__init__(self, num_trees, VSSampling(num_trees), KernelLSH(), num, branch, granularity)
		else:
			LSHForest.__init__(self, num_trees, VSSampling(num_trees), E2LSH(norm=2), num, branch, distance.euclidean, granularity)